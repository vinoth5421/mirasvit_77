...
class Result extends Template
{
    ...
    public function getAggregationData(): array
    {
        ...
        if (!$this->config->isSearchIn() || empty($categoryAggregation)) {
            return [];
        }
    ...
    private function processCategoryAggregation(array $categoryAggregaition): array
    {
        $result     = [];
        ...
        $categoriesCollection = $this->categoryCollectionFactory->create()
            ...
            ->addFieldToFilter('entity_id', ['in' => array_keys($categoryAggregaition)])
            ->addFieldToFilter('is_anchor', ['eq' => 1])
            ->setStoreId('field(' . implode(', ', array_keys($categoryAggregaition)) . ')')
            ->setPageSize(10);

        $categoriesCollection->getSelect()->order(new \Zend_Db_Expr('FIELD(e.entity_id,'. join(',', array_keys($categoryAggregaition)).')'));
        ...
        foreach ($categoriesCollection as $category) {
            ...
            if (!isset($result[$category->getName()])) {
                $result[$category->getName()] = [
                    ...
                    'url'       => $url,
                    'count'     => $categoryAggregaition[$category->getId()]['count'],
                ];
            } else {
